...
public class OpenApiConfig {
    ...
    public OpenAPI openAPI(@Value("${springdoc.version}") String springdocVersion) {
        Info info = new Info()
                .title("MONO API Document")
                .version(springdocVersion)
                .description("API Document for MONO Service ");
        SecurityScheme securityScheme = new SecurityScheme()
                .type(SecurityScheme.Type.HTTP)
                .scheme(StaticStr.TOKEN_PREFIX.trim())
                .bearerFormat("JWT")
                .description("")
                .in(SecurityScheme.In.HEADER).name(StaticStr.HEADER_STRING);
